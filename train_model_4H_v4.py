...
# [V4] PRICE_FEATS: all dimensionless (price-relative), will be shift(1)-ed.
# TIME_FEATS: temporal encodings, represent CURRENT bar → never shifted.
PRICE_FEATS = [
...
print(f"[config] 4H-v4 | SEQ={SEQ_LEN} | NF={NF} (price={N_PRICE}/time={len(TIME_FEATS)}) | TRIALS={N_TRIALS}", flush=True)
...
def mkfeat(btc_df, eth_df):
    ...
    # [V4] EMAs stored as price-relative ratios (dimensionless, scale-invariant)
    ...
    # [V4] MACD normalized by price (dimensionless)
    ...
    # [V4] Candle structure normalized by ATR (dimensionless)
    ...
    # [V4] BB bands as distance from price, normalized
    ...
    # TIME_FEATS: computed AFTER price features, no shift applied
    ...
    # [V4] ANTI-LEAKAGE: shift all price/indicator features by 1 bar.
    # At prediction bar t, the model only sees indicators finalised at bar t-1.
    # TIME_FEATS intentionally excluded: they encode WHEN bar t occurs (entry time).
...
def build_seqs(d_arr, c_arr, vol_arr, atr_arr, sl=SEQ_LEN, fs=FUTURE_STEPS):
    ...
    # [V4] ATR at entry bar (last bar of each sequence) for stop-loss
...
def _glu(x, units, d_rate):
    ...
    g_ = Lambda(lambda t: tf.cast(tf.sigmoid(tf.cast(t[..., units:], tf.float32)), t.dtype))(h)
...
def _temporal_gate(x, units, d_rate):
    ...
    weights = Lambda(lambda t: tf.cast(tf.nn.softmax(tf.cast(t, tf.float32), axis=1), t.dtype))(scores)
    ctx     = Lambda(lambda t: tf.reduce_sum(t[0] * t[1], axis=1))([x, weights])
...
def backtest(pr, y_norm, y_raw, atr_arr, atr_mult=ATR_MULT):
    ...
    # [V4] Dynamic ATR stop-loss: cap loss per trade in log-return space
    ...
    try:
        ...
        if np.isfinite(val):
            ...
            TRIAL_RESULTS[tn] = {
                ...
                "sharpe": bt["sharpe"], "mdd": bt["mdd"],
                "pf": bt["pf"], "model": ckpt,
            }
            print(
                f"[T{tn:02d}] loss={val:.6f} | Ret={bt['ret']:.2%} | "
                f"Sharpe={bt['sharpe']:.2f} | PF={bt['pf']:.2f} | MDD={bt['mdd']:.2%}",
                flush=True,
            )
...
# ── [V4] Best-Trial Selector: Sharpe + PF weighted, filters negative Sharpe ──

def find_best_trial():
    if not TRIAL_RESULTS:
        print("[tracker] No trials.", flush=True)
        return None
    trials  = list(TRIAL_RESULTS.keys())
    # Primary filter: positive Sharpe AND drawdown within tolerance
    valid   = [t for t in trials
               if TRIAL_RESULTS[t]["sharpe"] > 0 and TRIAL_RESULTS[t]["mdd"] > -0.60]
    if not valid:
        print("[tracker] No trial passed filter – selecting top-5% by Sharpe.", flush=True)
    ...
    # [V4] Sharpe 50% + PF 30% + Loss 20%  (was Loss 40% in V3)
    score    = 0.50 * _norm(sharpes) + 0.30 * _norm(pfs) + 0.20 * _norm(losses, lower_better=True)
    ...
    print(
        f"  loss={best['val_loss']:.6f} | Sharpe={best['sharpe']:.2f} | "
        f"PF={best['pf']:.2f} | MDD={best['mdd']:.2%}",
        flush=True,
    )
    print("=" * 72, flush=True)
    return best["model"]
...
def train():
    print(f"[TFT-4H-v4] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | TRIALS={N_TRIALS}", flush=True)
    ...
    # [V4] Compute UNSHIFTED ATR-norm separately for stop-loss (entry bar ATR).
    # This is distinct from the shifted atr_norm inside the feature matrix.
    ...
    atr_s   = atr_norm_u.reindex(feat_df.index).ffill()
    del btc_raw, eth_raw, atr_norm_u; gc.collect()

    T       = len(feat_df)
    n_seqs  = T - SEQ_LEN - FUTURE_STEPS
    tr_cut  = int(n_seqs * TRAIN_FRAC)
    val_cut = int(n_seqs * (TRAIN_FRAC + VAL_FRAC))

    # [V4] Scaler fitted ONLY on PRICE_FEATS of training rows; TIME_FEATS excluded
    # (already in bounded [-1,1] / [0,1] ranges, no scaling needed).
    scaler_end = tr_cut + SEQ_LEN
    feat_arr   = feat_df.values.astype(np.float32)
    del feat_df; gc.collect()

    sc = RobustScaler()
    feat_arr[:scaler_end, :N_PRICE] = sc.fit_transform(feat_arr[:scaler_end, :N_PRICE])
    feat_arr[scaler_end:, :N_PRICE] = sc.transform(feat_arr[scaler_end:, :N_PRICE])
    gc.collect()
    ...
    print(
        f"[seqs] train={len(Xtr):,} | val={len(Xv):,} | test={len(Xte):,} | "
        f"sl={sl} | nf={nf_}",
        flush=True,
    )
    ...
    study.optimize(
        lambda t: objective(t, Xtr, ytr, Xv, yv, yr_v, atr_v),
    ...
    joblib.dump(
        {"sc": sc, "N_PRICE": N_PRICE, "ALL_FEATS": ALL_FEATS, "PRICE_FEATS": PRICE_FEATS},
