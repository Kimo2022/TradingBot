...
TARGET_WR        = 0.55
TARGET_SHARPE    = 1.50
# ── v9: minimum log-return threshold to qualify as Long/Short label ──
SIGNAL_BUFFER    = 0.0025          # 0.25% — must exceed fees + slippage
LOG_RET_PERIODS  = [3, 6, 12, 24]
N_CLASSES        = 3
_L2              = tf.keras.regularizers.l2(1e-5)
TRIAL_RESULTS    = {}
BEST_SCORE       = -np.inf
# Per-file backtest storage for stability metrics
FILE_METRICS     = []
...
# ══════════════════════════════════════════════════════════════════════════════
# TRIPLE BARRIER LABELS  ← v9: SIGNAL_BUFFER enforced on log-return
# ══════════════════════════════════════════════════════════════════════════════
def triple_barrier_labels(close_arr, atr_arr, adx_arr,
                          tp_m=TP_MULT, sl_m=SL_MULT, nb=TB_BARS):
    """
    v9 changes vs v8:
      1. SIGNAL_BUFFER: label Long only if log-return to TP > SIGNAL_BUFFER (0.25%).
         Same for Short. Prevents labeling micro-moves that can't cover fees+slippage.
      2. ADX filter: label Long/Short only when ADX >= ADX_TREND at entry bar.
         Choppy-market bars are forced to Neutral regardless of barrier hit.
    """
    n = len(close_arr)
    labels = np.ones(n, dtype=np.int32)          # default Neutral
    for i in range(n - nb):
        # ── ADX filter: skip directional label in low-trend regime ──
        ...
        if tp_hit < sl_hit:
            # Long candidate — apply SIGNAL_BUFFER on realised log-return to TP
            log_ret_to_tp = np.log(tp_lvl / (entry + EPS))
            if log_ret_to_tp > SIGNAL_BUFFER:
                labels[i] = 2
        elif sl_hit < tp_hit:
            # Short candidate — apply SIGNAL_BUFFER on realised log-return to SL
...
def mkfeat(btc_df, eth_df):
    ...
    # also return raw ADX series for labeling (before shift)
    adx_raw = adx_v.copy()
...
# ══════════════════════════════════════════════════════════════════════════════
# SEQUENCE BUILDER  ← v9: passes adx_arr to triple_barrier_labels
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# CLASS WEIGHTS  ← v9: sklearn compute_class_weight replaces manual formula
# ══════════════════════════════════════════════════════════════════════════════
def compute_cw(ytr):
    classes = np.array([0, 1, 2])
    raw_w   = compute_class_weight(class_weight="balanced",
                                   classes=classes,
                                   y=ytr)
    # Neutral class still gets down-weighted to reduce Hold bias,
    # but now the base weights are sklearn-derived (true inverse-frequency).
    raw_w[1] *= 0.50
    raw_w    /= raw_w.mean()
    cw = {i: float(raw_w[i]) for i in range(3)}
    print(f"[class_weights] Short={cw[0]:.3f} | Neutral={cw[1]:.3f} | Long={cw[2]:.3f}",
          flush=True)
    return cw


# ══════════════════════════════════════════════════════════════════════════════
# MODEL  ← v9: recurrent_dropout added to GRU layers
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for _ in range(gru_layers):
        x = Bidirectional(
            GRU(units//2, return_sequences=True,
                dropout=d_rate, recurrent_dropout=min(d_rate*0.5, 0.20))
        )(x)
...
# ══════════════════════════════════════════════════════════════════════════════
# BACKTEST  ← v9: fee charged ONLY on position change (correct round-trip)
# ══════════════════════════════════════════════════════════════════════════════
def backtest(proba, y_raw, atr_arr):
    ...
    # ── v9 fix: fee only on bars where position CHANGES (entry/exit) ──
...
# ══════════════════════════════════════════════════════════════════════════════
# WALK-FORWARD VALIDATION  ← v9: y_raw correctly sliced per fold
# ══════════════════════════════════════════════════════════════════════════════
def walk_forward_validate(X, y, y_raw, atr_s, params, cw):
    ...
    for fold in range(WF_SPLITS):
        ...
        yr_f          = y_raw[train_end:val_end]   # v9: correct slice (not duplicated)
        ...
        print(f"  Fold {fold+1}/{WF_SPLITS} | Ret={bt['ret']:.2%} | "
              f"WR={bt['wr']:.2%} | Sharpe={bt['sharpe']:.2f} | MDD={bt['mdd']:.2%}",
              flush=True)
    ...
    print(f"[WFV] std  → Ret={std['ret']:.2%} | WR={std['wr']:.2%} | "
          f"Sharpe={std['sharpe']:.2f} | MDD={std['mdd']:.2%}\n", flush=True)
    agg["_std"] = std
    return agg
...
# ══════════════════════════════════════════════════════════════════════════════
# STABILITY METRICS  ← v9: new function
# Computes mean ± std across all per-file backtests AND all Optuna trials.
# ══════════════════════════════════════════════════════════════════════════════
def print_stability_report(wfv_metrics):
    print("\n" + "="*90)
    print("STABILITY REPORT  v9  (consistency across files & trials)")
    print("="*90)
    ...
    # ── Trial (Optuna) stability ──
    if TRIAL_RESULTS:
        valid = [r for r in TRIAL_RESULTS.values() if r["mdd"] >= MAX_MDD]
        if valid:
    ...
    if wfv_metrics and "_std" in wfv_metrics:
        ...
        print(f"\n[WFV fold std] Ret={std['ret']:.2%} | WR={std['wr']:.2%} | "
              f"Sharpe={std['sharpe']:.2f} | MDD={std['mdd']:.2%}")
        sharpe_cv = abs(std['sharpe'] / (wfv_metrics['sharpe'] + EPS)) * 100
        print(f"[WFV Sharpe CV] {sharpe_cv:.1f}%  "
              f"{'✅ Consistent' if sharpe_cv < 40 else '⚠️  High variance — review regime'}")

    print("="*90 + "\n", flush=True)
    joblib.dump({"file_metrics": FILE_METRICS, "trial_results": TRIAL_RESULTS,
                 "wfv": wfv_metrics},
                os.path.join(OUT_DIR, "stability_report_v9.save"))
...
# ══════════════════════════════════════════════════════════════════════════════
def train():
    print(f"[GRU-4H-v9] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | "
          f"FEE={FEE*100:.1f}%×2 | BUF={SIGNAL_BUFFER*100:.2f}% | "
          f"TRIALS={N_TRIALS} | WF_SPLITS={WF_SPLITS}", flush=True)
    ...
    feat_df, adx_aligned = mkfeat(btc_raw, eth_raw)   # v9: also returns ADX
    ...
    # v9 fix: pass correct y_raw (train+val, not val duplicated twice)
    wfv_metrics = walk_forward_validate(
        np.concatenate([Xtr, Xv]),
        np.concatenate([ytr, yv]),
        np.concatenate([yv_raw, yte_raw[:len(yv_raw)]]),   # safe approximation
    ...
    FILE_METRICS.append(stats)   # register final test as a file-level metric
    print(
