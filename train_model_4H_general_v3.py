...
BASE_FEATS = [
    ...
    "price_range_norm","atr_pct_ret",
    "body","up_wick","lo_wick",
    "bb_up","bb_lo","bb_w",
    "vol_ema","tkr_ratio","rvi","rvi_slope","vol_trend","vpt_norm",
...
NF        = len(ALL_FEATS)
print(f"[config] 4H-v3 | SEQ={SEQ_LEN} | NF={NF} | TRIALS={N_TRIALS}", flush=True)
...
def mkfeat(btc_df, eth_df):
    ...
    df["vol_trend"] = (v.ewm(span=12, adjust=False).mean() /
                       (v.ewm(span=42, adjust=False).mean() + EPS))
    ...
    df["close_above_ema200"] = (c > df["ema200"]).astype(np.float32)
...
def build_seqs(d_arr, c_arr, vol_arr, sl=SEQ_LEN, fs=FUTURE_STEPS):
...
def objective(trial, Xtr, ytr, Xv, yv, yv_raw):
...
def train():
    print(f"[TFT-4H-v3] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | TRIALS={N_TRIALS}", flush=True)
    ...
    # LEAKAGE-FREE: scaler fitted only on feature rows consumed by training sequences
    # Training sequences [0, tr_cut) use feature rows [0, tr_cut + SEQ_LEN - 1]
    scaler_end = tr_cut + SEQ_LEN
    feat_arr   = feat_df.values.astype(np.float32)
    sc         = RobustScaler()
    feat_arr[:scaler_end] = sc.fit_transform(feat_arr[:scaler_end])
    feat_arr[scaler_end:] = sc.transform(feat_arr[scaler_end:])
    ...
    X, y, y_raw = build_seqs(feat_arr, c_arr, vol_arr)
    del feat_arr, c_arr, vol_arr; gc.collect()
    ...
    study.optimize(
        lambda t: objective(t, Xtr, ytr, Xv, yv, yr_v),
