...
# ══════════════════════════════════════════════════════════════════════════════
# TRIPLE BARRIER LABELS  — v10: ADX_TREND=18, SIGNAL_BUFFER=0.006
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# BARRIER RETURNS — v10: numba-accelerated, replaces single-bar y_raw
# ══════════════════════════════════════════════════════════════════════════════
@njit(cache=True)
def _compute_barrier_returns_nb(close_arr, atr_arr, sl, n_seqs,
    ...
    for i in range(n_seqs):
        ...
        for j in range(1, nb + 1):
            p = close_arr[bar + j]
            if tp_hit == nb and p >= tp_lvl:
                tp_hit = j - 1
            if sl_hit == nb and p <= sl_lvl:
                sl_hit = j - 1
...
def compute_barrier_returns(close_arr, atr_arr, sl=SEQ_LEN, n_seqs=None):
    ...
    close_f64 = close_arr.astype(np.float64)
    atr_f32   = atr_arr.astype(np.float32)
    return _compute_barrier_returns_nb(
        close_f64, atr_f32, sl, n_seqs,
        float(TP_MULT), float(SL_MULT), int(TB_BARS), EPS
    )
...
# ══════════════════════════════════════════════════════════════════════════════
# SEQUENCE BUILDER  — v10: y_raw = barrier return (not single-bar)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# BACKTEST  — v10: barrier returns + conf threshold 0.55
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# COMPOSITE SCORE  — v10: PF وزن 30%، bonus على PF≥1.5
# ══════════════════════════════════════════════════════════════════════════════
...
def print_stability_report(wfv_metrics):
    print("\n" + "="*90)
    print("STABILITY REPORT  v10  (consistency across files & trials)")
    print("="*90)
    if FILE_METRICS:
    ...
    joblib.dump({"file_metrics": FILE_METRICS, "trial_results": TRIAL_RESULTS,
                 "wfv": wfv_metrics},
                os.path.join(OUT_DIR, "stability_report_v10.save"))
...
# ══════════════════════════════════════════════════════════════════════════════
def train():
    print(f"[GRU-4H-v10] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | "
          f"FEE={FEE*100:.1f}%×2 | BUF={SIGNAL_BUFFER*100:.2f}% | "
          f"MAX_MDD={MAX_MDD:.0%} | ADX≥{ADX_TREND} | "
