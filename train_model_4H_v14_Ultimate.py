...
from tensorflow.keras.callbacks import (
    ModelCheckpoint, TerminateOnNaN, ReduceLROnPlateau,
)
...
def triple_barrier_labels(close_arr, atr_arr, adx_arr,
    ...
    intrabar = np.zeros(n, dtype=bool)
    for i in range(n - nb):
        if adx_arr[i] < ADX_TREND:
            continue
        entry   = close_arr[i]
        atr_i   = float(atr_arr[i])
        dyn_fee = min(max(DYN_FEE_BASE + atr_i, DYN_FEE_FLOOR), DYN_FEE_CAP)
        buf     = 2.0 * dyn_fee * MIN_EDGE_MULTIPLE
        tp_lvl  = entry * (1 + tp_m * atr_i)
        sl_lvl  = entry * (1 - sl_m * atr_i)
        ...
        if tp_hit == sl_hit and tp_hit < nb:
            labels[i]    = 0
            intrabar[i]  = True
        elif tp_hit < sl_hit:
            if np.log(tp_lvl / (entry + EPS)) > buf:
                labels[i] = 2
        elif sl_hit < tp_hit:
            if np.log(entry / (sl_lvl + EPS)) > buf:
                labels[i] = 0
    return labels, intrabar


@njit(cache=True)
def _compute_barrier_returns_nb(close_arr, atr_arr, sl, n_seqs,
                                  tp_m, sl_m, nb, eps,
                                  fee_base, fee_floor, fee_cap):
    ...
    for i in range(n_seqs):
        ...
        if entry <= 0.0:
            continue
        atr_i   = atr_arr[bar]
        dyn_fee = min(max(fee_base + atr_i, fee_floor), fee_cap)
        tp_lvl  = entry * (1.0 + tp_m * atr_i)
        sl_lvl  = entry * (1.0 - sl_m * atr_i)
        tp_hit  = nb; sl_hit = nb
...
def compute_barrier_returns(close_arr, atr_arr, sl=SEQ_LEN, n_seqs=None):
    ...
    return _compute_barrier_returns_nb(
        ...
        DYN_FEE_BASE, DYN_FEE_FLOOR, DYN_FEE_CAP,
    )
...
def mkfeat(btc_df, eth_df):
    ...
    v30  = df["volatility"].rolling(180).mean()
    v30s = df["volatility"].rolling(180).std()
...
def fit_scale_inplace(feat_arr_log, train_end_raw):
    sc = RobustScaler()
    feat_arr_log[:train_end_raw, :N_PRICE] = sc.fit_transform(
        feat_arr_log[:train_end_raw, :N_PRICE])
    feat_arr_log[train_end_raw:, :N_PRICE] = sc.transform(
        feat_arr_log[train_end_raw:, :N_PRICE])
    return sc


def compute_sample_weights(y_raw, intrabar_mask):
    w = np.clip(np.abs(y_raw) * 80.0, 0.3, MAX_SAMPLE_WEIGHT).astype(np.float32)
    if intrabar_mask is not None:
        w[intrabar_mask] = MAX_SAMPLE_WEIGHT
    return w

def select_features(X_train, y_raw_train, threshold=MIN_FEAT_CORR):
    n_feat = X_train.shape[2]
    n_sub  = min(len(X_train), 5000)
    rng    = np.random.default_rng(SEED)
    idx    = rng.choice(len(X_train), n_sub, replace=False)
    last   = X_train[idx, -1, :]
    y_sub  = y_raw_train[idx]
    disabled = []
    for fi in range(n_feat):
        try:
            col = last[:, fi]
            if col.std() < 1e-7:
                disabled.append(fi); continue
            corr, _ = spearmanr(col, y_sub)
...
def build_seqs(feat_arr, close_arr, atr_arr, adx_arr,
               disabled_feats=None, sl=SEQ_LEN):
    n_seqs = len(feat_arr) - sl - TB_BARS
    idx    = np.arange(n_seqs)[:, None] + np.arange(sl)
    X      = feat_arr[idx]
    if disabled_feats:
        X = X.copy(); X[:, :, disabled_feats] = 0.0
    labels, intrabar     = triple_barrier_labels(close_arr, atr_arr, adx_arr)
    y                    = labels[:n_seqs].astype(np.int32)
    ib                   = intrabar[:n_seqs]
    y_raw, _             = compute_barrier_returns(close_arr, atr_arr, sl, n_seqs)
    atr_s                = atr_arr[sl : sl+n_seqs]
    sw                   = compute_sample_weights(y_raw, ib)
    return X, y, y_raw, atr_s, sw
...
class TemporalAttentionPooling(tf.keras.layers.Layer):
    def __init__(self, recent_bias=2.0, **kwargs):
        ...
        self._score      = Dense(1, use_bias=True)

    def call(self, x, training=None):
        seq_len = tf.shape(x)[1]
        scores  = self._score(x)
        pos     = tf.cast(tf.range(seq_len), tf.float32) / tf.cast(seq_len, tf.float32)
        pos     = tf.reshape(pos, (1, -1, 1))
        scores  = scores + self.recent_bias * pos
        weights = tf.nn.softmax(scores, axis=1)
        return tf.reduce_sum(x * weights, axis=1)

    def get_config(self):
        cfg = super().get_config()
        cfg.update({"recent_bias": self.recent_bias})
        return cfg
...
def build_model(seq_len, n_feat, units, nh, d_rate, lr, noise, gru_layers=2):
    ...
    for _ in range(gru_layers):
        ...
        x = LayerNormalization()(Add()([gru, res]))

    attn = MultiHeadAttention(
        num_heads=nh,
        key_dim=max(units//nh, 8),
        dropout=d_rate
    )(x, x)
    x = LayerNormalization()(Add()([x, attn]))

    x   = TemporalAttentionPooling(recent_bias=2.0)(x)
...
def make_ds(X, y, w, batch, shuffle=False):
    ds = tf.data.Dataset.from_tensor_slices(
        (X.astype(np.float32), y.astype(np.int32), w.astype(np.float32))
    )
...
def backtest(proba, y_raw, atr_arr):
    _default = dict(
        ret=0.0, mdd=0.0, sharpe=0.0, sortino=0.0, pf=0.0, wr=0.0,
        trades=0, top_ratio=0.0,
        long_wr=0.0, short_wr=0.0, long_ret=0.0, short_ret=0.0,
    ...
    try:
        ...
        if len(pos_rets) >= TOP_K_RATIO:
            top_k     = np.sort(pos_rets)[-TOP_K_RATIO:]
            top_ratio = float(top_k.sum() / (pos_rets.sum() + EPS))
        elif len(pos_rets) > 0:
            top_ratio = float(pos_rets.max() / (pos_rets.sum() + EPS))
        else:
            top_ratio = 0.0

        long_mask    = (pred == 2) & active
        short_mask   = (pred == 0) & active
        long_rets_v  = rets[long_mask]
        short_rets_v = rets[short_mask]
        ...
        return dict(
            ret=float(cum[-1]-1), mdd=mdd, sharpe=sh, sortino=sortino,
            pf=pf, wr=wr, trades=n_tr, top_ratio=top_ratio,
            long_wr=long_wr, short_wr=short_wr,
            long_ret=long_ret, short_ret=short_ret,
            long_trades=int(np.count_nonzero(long_mask)),
            short_trades=int(np.count_nonzero(short_mask)),
        )
...
def _composite_score_v14(bt, wfv_scores=None, val_loss=0.0):
    if bt["trades"] == 0:
        return -2.0
...
class FoldScoreEarlyStopping(tf.keras.callbacks.Callback):
    def __init__(self, Xv, yv_raw, atr_v, patience=8, min_delta=0.001):
        super().__init__()
        self._Xv          = Xv
        self._yr          = yv_raw
        self._at          = atr_v
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        fs = _fold_score(bt) - 0.1 * self.last_val_loss
        if fs > self.best_score + self.min_delta:
            self.best_score   = fs
            self.wait         = 0
...
def _try_save_best(model, score, label=""):
    ...
    if np.isfinite(score) and score > BEST_SCORE:
        BEST_SCORE = score
        model.save(os.path.join(OUT_DIR, "best_model_v14.keras"))
        print(f"  ★ [{label}] NEW BEST  score={score:.4f} → best_model_v14.keras", flush=True)


def objective(trial, Xtr, ytr, sw_tr, Xv, yv, sw_v, yv_raw, atr_v):
    ...
    try:
        ...
        ds_tr  = make_ds(Xtr, ytr, sw_tr, BATCH, shuffle=True)
        ds_vl  = make_ds(Xv,  yv,  sw_v,  BATCH)
        fs_cb  = FoldScoreEarlyStopping(Xv, yv_raw, atr_v, patience=8)
        cb = [
        ...
        final_val_loss = fs_cb.last_val_loss
        ...
        score = _composite_score_v14(bt, wfv_scores=None, val_loss=final_val_loss)
        opt_score = -score

        if np.isfinite(score):
            ckpt = os.path.join(OUT_DIR, f"model_v14_trial_{tn:02d}.keras")
            m.save(ckpt)
            ...
            TRIAL_RESULTS[tn] = {
                ...
                "long_wr": bt["long_wr"], "short_wr": bt["short_wr"],
            ...
            print(
                ...
                f"L={bt['long_wr']:.2%}|S={bt['short_wr']:.2%}|"
                f"Sortino={bt['sortino']:.2f}|MDD={bt['mdd']:.2%}|"
                f"TR5={bt['top_ratio']:.3f}|score={score:.4f} {mdd_tag}", flush=True,
            )
...
def walk_forward_validate(feat_arr_log, c_arr, atr_arr, adx_arr, n_wfv, params, cw):
    wf_val_sz   = n_wfv // (WF_SPLITS + 1)
    wf_train_sz = wf_val_sz * WF_TRAIN_RATIO
    wf_metrics  = []
    fold_scores = []
    print(f"\n[WFV-v14] {WF_SPLITS} folds | val_sz≈{wf_val_sz:,} | "
          f"train_sz≈{wf_train_sz:,} | sliding-window ✓ | scaler-isolated ✓ | feat-sel ✓",
          flush=True)

    for fold in range(WF_SPLITS):
        val_start   = wf_val_sz * (fold + 1)
        val_end     = val_start + wf_val_sz
        train_start = max(0, val_start - wf_train_sz)
        train_end   = val_start

        if val_end > n_wfv:
            print(f"  Fold {fold+1}: val_end {val_end} > n_wfv {n_wfv} — skipping.", flush=True)
            break

        n_tr_seqs = train_end - train_start
        n_val     = val_end - val_start

        if n_val < 200 or n_tr_seqs < 500:
            print(f"  Fold {fold+1}: seqs insufficient (tr={n_tr_seqs} val={n_val}) — skipping.",
                  flush=True)
            continue

        train_raw_end = train_end + SEQ_LEN
        val_raw_end   = val_end + SEQ_LEN + TB_BARS
        total_raw     = min(val_raw_end, len(feat_arr_log))
        raw_start     = train_start

        fold_feat = feat_arr_log[raw_start:total_raw].copy()
        sc_fold   = RobustScaler()
        local_tr_raw_end = train_raw_end - raw_start

        fold_feat[:local_tr_raw_end, :N_PRICE] = sc_fold.fit_transform(
            fold_feat[:local_tr_raw_end, :N_PRICE])
        fold_feat[local_tr_raw_end:, :N_PRICE] = sc_fold.transform(
            fold_feat[local_tr_raw_end:, :N_PRICE])

        fold_c   = c_arr[raw_start:total_raw]
        fold_atr = atr_arr[raw_start:total_raw]
        if len(adx_arr) >= total_raw:
            fold_adx = adx_arr[raw_start:total_raw]
        else:
            fold_adx = np.concatenate([
                adx_arr[raw_start:],
                np.full(total_raw-len(adx_arr), adx_arr[-1], dtype=np.float32)
            ])

        X_f, y_f, yr_f, at_f, sw_f = build_seqs(fold_feat, fold_c, fold_atr, fold_adx)
        del fold_feat; gc.collect()

        local_tr_end  = n_tr_seqs
        local_val_end = n_tr_seqs + n_val

        if len(X_f) < local_val_end:
            print(f"  Fold {fold+1}: built {len(X_f)} seqs < {local_val_end} needed — skipping.",
                  flush=True)
            del X_f, y_f, yr_f, at_f, sw_f; gc.collect()
            continue

        Xtr_f = X_f[:local_tr_end];                ytr_f  = y_f[:local_tr_end]
        sw_tr_f = sw_f[:local_tr_end]
        Xv_f  = X_f[local_tr_end:local_val_end];   yv_f   = y_f[local_tr_end:local_val_end]
        yr_vf = yr_f[local_tr_end:local_val_end];  at_vf  = at_f[local_tr_end:local_val_end]
        sw_vf = sw_f[local_tr_end:local_val_end]

        disabled = select_features(Xtr_f, yr_f[:local_tr_end])
        if disabled:
            Xtr_f[:, :, disabled] = 0.0
            Xv_f[:, :, disabled]  = 0.0
            print(f"  Fold {fold+1}: disabled {len(disabled)}/{NF} weak features", flush=True)
        ...
        ds_tr = make_ds(Xtr_f, ytr_f, sw_tr_f, BATCH, shuffle=True)
        ds_vl = make_ds(Xv_f,  yv_f,  sw_vf,   BATCH)
        fs_cb = FoldScoreEarlyStopping(Xv_f, yr_vf, at_vf, patience=8)
        cb = [
            ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=4,
                              min_lr=1e-7, verbose=0),
            TerminateOnNaN(),
        ...
        fold_val_loss = fs_cb.last_val_loss
        pr            = mf.predict(make_ds_eval(Xv_f, BATCH), verbose=0)
        n_aligned     = min(len(pr), len(yr_vf))
        ...
        mdd_tag = "🔴" if bt["mdd"] < MAX_MDD else ("⚠️" if bt["mdd"] < MDD_SOFT else "✅")
        print(
            f"  Fold {fold+1}/{WF_SPLITS} | Ret={bt['ret']:.2%} | WR={bt['wr']:.2%} | "
            f"L={bt['long_wr']:.2%}({bt['long_trades']}) "
            f"S={bt['short_wr']:.2%}({bt['short_trades']}) | "
    ...
    print(
        f"[WFV] mean → Ret={agg['ret']:.2%} | WR={agg['wr']:.2%} | "
        f"L={agg['long_wr']:.2%} | S={agg['short_wr']:.2%} | "
...
def print_summary_table():
    ...
    print("\n" + "="*130)
    print("TRIAL SUMMARY  v14-Ultimate  (🔴=MDD>25% | ⚠️=MDD>18% | ✅=OK)")
    print("="*130)
    print(df.to_string())
    print("="*130)
    if best_tn:
        ...
        print(
            f"★ BEST: T{best_tn:02d} | Score={b.get('score',0):.4f} | "
            f"Ret={b.get('ret',0):.2%} | WR={b.get('wr',0):.2%} | "
            f"L={b.get('long_wr',0):.2%} | S={b.get('short_wr',0):.2%} | "
            f"Sortino={b.get('sortino',0):.2f} | MDD={b.get('mdd',0):.2%} | "
            f"NW={b.get('neutral_weight',0):.2f} | Top5R={b.get('top_ratio',0):.3f}"
        )
    print("="*130 + "\n", flush=True)
    joblib.dump(df, os.path.join(OUT_DIR, "trial_summary_v14.save"))


def print_stability_report(wfv_metrics):
    print("\n" + "="*90)
    print("STABILITY REPORT  v14-Ultimate")
    print("="*90)
    if FILE_METRICS:
    ...
    if wfv_metrics and "_std" in wfv_metrics:
        ...
        print(
            f"\n[L/S Split WFV]  "
            f"Long_WR={wfv_metrics.get('long_wr',0):.2%} | "
            f"Short_WR={wfv_metrics.get('short_wr',0):.2%} | "
            f"Long_Trades={wfv_metrics.get('long_trades',0):.0f} | "
            f"Short_Trades={wfv_metrics.get('short_trades',0):.0f} | "
            f"Long_Ret={wfv_metrics.get('long_ret',0):.4f} | "
            f"Short_Ret={wfv_metrics.get('short_ret',0):.4f}"
        )
    print("="*90 + "\n", flush=True)
    joblib.dump(
        {"file_metrics": FILE_METRICS, "trial_results": TRIAL_RESULTS, "wfv": wfv_metrics},
        os.path.join(OUT_DIR, "stability_report_v14.save"),
    )


def train():
    print(
        f"[BiGRU-4H-v14-Ultimate] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | "
        f"DynFee=[{DYN_FEE_FLOOR*100:.2f}%-{DYN_FEE_CAP*100:.2f}%] | "
        f"MDD_HARD={MAX_MDD:.0%} | MDD_SOFT={MDD_SOFT:.0%} | ADX≥{ADX_TREND} | "
        f"TRIALS={N_TRIALS} | WF_SPLITS={WF_SPLITS} | WF_TRAIN_RATIO={WF_TRAIN_RATIO}× | "
        f"ResidualBiGRU ✓ | TemporalAttn ✓ | SlidingWFV ✓ | IntrabarReality ✓ | "
        f"AdaptiveBuf ✓ | Top{TOP_K_RATIO}Ratio ✓ | FeatSel ✓ | SampleWeights ✓",
        flush=True,
    )
    ...
    sc_global        = fit_scale_inplace(feat_arr_optuna, train_end_raw)
    gc.collect()
    ...
    X, y, y_raw, atr_seqs, sw = build_seqs(feat_arr_optuna, c_arr, atr_arr, adx_aligned)
    del feat_arr_optuna; gc.collect()

    Xtr = X[:tr_cut];         ytr = y[:tr_cut];  sw_tr = sw[:tr_cut]
    Xv  = X[tr_cut:val_cut];  yv  = y[tr_cut:val_cut]; sw_v = sw[tr_cut:val_cut]
    ...
    study.optimize(
        lambda t: objective(t, Xtr, ytr, sw_tr, Xv, yv, sw_v, yv_raw, atr_v),
    ...
    wfv_params = dict(
        units=bp["units"], nh=bp["nh"], d_rate=bp["d"],
    ...
    ds_tr    = make_ds(Xtr, ytr, sw_tr, BATCH, shuffle=True)
    ds_vl    = make_ds(Xv,  yv,  sw_v,  BATCH)
    fs_final = FoldScoreEarlyStopping(Xv, yv_raw, atr_v, patience=10)
    cbs = [
        ModelCheckpoint(os.path.join(OUT_DIR, "model_v14_best_final.keras"),
    ...
    final_score     = _composite_score_v14(stats, wfv_scores=wfv_fold_scores,
                                            val_loss=final_val_loss)

    print(
        f"\n[FINAL TEST] Ret={stats['ret']:.2%} | WR={stats['wr']:.2%} | "
        f"L={stats['long_wr']:.2%}({stats['long_trades']}) | "
        f"S={stats['short_wr']:.2%}({stats['short_trades']}) | "
        f"Sharpe={stats['sharpe']:.2f} | Sortino={stats['sortino']:.2f} | "
        f"MDD={stats['mdd']:.2%} | PF={stats['pf']:.2f} | "
        f"Trades={stats['trades']} | Top5Ratio={stats['top_ratio']:.3f} | "
        f"ValLoss={final_val_loss:.4f} | FinalScore={final_score:.4f}",
        flush=True,
    )
    if wfv_metrics:
        print(
            f"[WFV  MEAN]  Ret={wfv_metrics.get('ret',0):.2%} | "
            f"WR={wfv_metrics.get('wr',0):.2%} | "
            f"L={wfv_metrics.get('long_wr',0):.2%} | "
            f"S={wfv_metrics.get('short_wr',0):.2%} | "
            f"Sortino={wfv_metrics.get('sortino',0):.2f} | "
            f"Sharpe={wfv_metrics.get('sharpe',0):.2f} | "
            f"MDD={wfv_metrics.get('mdd',0):.2%}",
            flush=True,
        )

    print_stability_report(wfv_metrics)

    m.save(os.path.join(OUT_DIR, "model_v14_final.keras"))
    _try_save_best(m, final_score, label="final")

    joblib.dump(
        {
            "sc": sc_global, "NF": NF, "N_PRICE": N_PRICE,
            "ALL_FEATS": ALL_FEATS, "PRICE_FEATS": PRICE_FEATS,
            "SEQ_LEN": SEQ_LEN, "best_params": bp,
            "DYN_FEE_BASE": DYN_FEE_BASE, "DYN_FEE_FLOOR": DYN_FEE_FLOOR,
            "DYN_FEE_CAP": DYN_FEE_CAP, "TOP_K_RATIO": TOP_K_RATIO,
        },
        os.path.join(OUT_DIR, "scaler_v14.pkl"),
    )
    joblib.dump(stats,         os.path.join(OUT_DIR, "stats_v14.save"))
    joblib.dump(wfv_metrics,   os.path.join(OUT_DIR, "wfv_metrics_v14.save"))
    joblib.dump(bp,            os.path.join(OUT_DIR, "best_params_v14.save"))
    joblib.dump(TRIAL_RESULTS, os.path.join(OUT_DIR, "trial_results_v14.save"))
