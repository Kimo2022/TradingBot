...
SEQ_LEN         = 60
BATCH           = 64
EPOCHS          = 100        # longer for 12h run
N_TRIALS        = 30         # more trials for 12h run
EPS             = 1e-8
FEE             = 0.001
TP_MULT         = 2.0
SL_MULT         = 1.0
...
WF_SPLITS       = 5
MAX_MDD         = -0.25
MDD_SOFT        = -0.20      # soft penalty threshold (new v11)
TARGET_WR       = 0.55
TARGET_SHARPE   = 1.50
MIN_EDGE_MULTIPLE = 3.0
SIGNAL_BUFFER   = round(2 * FEE * MIN_EDGE_MULTIPLE, 4)   # 0.0060
...
# ── FOCUSED LR & DROPOUT RANGES (from v10 winning zones) ─────────────────────
LR_LOW, LR_HIGH       = 5e-5, 1e-3   # narrowed from v10 best trials
D_LOW,  D_HIGH        = 0.20, 0.40   # narrowed from v10 best trials
...
# ══════════════════════════════════════════════════════════════════════════════
# BACKTEST — barrier returns + conf threshold 0.55
# ══════════════════════════════════════════════════════════════════════════════
def backtest(proba, y_raw, atr_arr):
    ...
    ret_std = rets.std()
    sh      = float(rets.mean() / (ret_std + EPS) * ANN_FACTOR)   # EPS guards div-by-zero
    neg     = rets[rets < 0]
    neg_std = neg.std() if len(neg) > 0 else EPS
    sortino = float(rets.mean() / (neg_std + EPS) * ANN_FACTOR)
    pos_sum = rets[rets>0].sum(); neg_sum = abs(neg.sum())
    pf      = float(pos_sum / (neg_sum + EPS))
...
# ══════════════════════════════════════════════════════════════════════════════
# COMPOSITE SCORE v11 — Drawdown Penalty + zero-div guard
# ══════════════════════════════════════════════════════════════════════════════
def _composite_score(bt):
    if bt["mdd"] < MAX_MDD:
        return -np.inf
    wr_score  = bt["wr"]
    mdd_raw   = bt["mdd"]        # negative number, e.g. -0.22
    # ── Drawdown penalty: double punishment if MDD exceeds soft limit ──────
    if mdd_raw < MDD_SOFT:       # worse than -20%
        mdd_score = 1.0 + 2.0 * mdd_raw   # steeper penalty
    else:
        mdd_score = 1.0 + mdd_raw
    mdd_score = max(mdd_score, 0.0)        # floor at 0
    pf_score  = min(bt["pf"], 5.0) / 5.0
    sh_raw    = bt["sharpe"]
    sh_score  = np.clip(sh_raw, -3.0, 5.0) / 5.0
    base  = 0.25*wr_score + 0.25*mdd_score + 0.30*pf_score + 0.20*sh_score
    bonus = (0.05 if bt["wr"]     >= TARGET_WR    else 0.0) + \
            (0.05 if bt["sharpe"] >= TARGET_SHARPE else 0.0) + \
            (0.05 if bt["pf"]     >= 1.5           else 0.0)
    return float(base + bonus)


# ══════════════════════════════════════════════════════════════════════════════
# GLOBAL BEST PRESERVATION — saves best_model_v11 + scaler_v11
# ══════════════════════════════════════════════════════════════════════════════
def _try_save_best(model, score, sc, label=""):
    global BEST_SCORE
    if score > BEST_SCORE and np.isfinite(score):
        BEST_SCORE = score
        model.save(os.path.join(OUT_DIR, "best_model_v11.keras"))
        joblib.dump(
            {"sc": sc, "NF": NF, "N_PRICE": N_PRICE,
             "ALL_FEATS": ALL_FEATS, "PRICE_FEATS": PRICE_FEATS,
             "SEQ_LEN": SEQ_LEN, "SIGNAL_BUFFER": SIGNAL_BUFFER},
            os.path.join(OUT_DIR, "scaler_v11.pkl"),
        )
        print(f"  ★ [{label}] NEW BEST  score={score:.4f} → best_model_v11.keras", flush=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# OPTUNA OBJECTIVE — focused search space from v10 winning zones
# ══════════════════════════════════════════════════════════════════════════════
def objective(trial, Xtr, ytr, Xv, yv, yv_raw, atr_v, cw, sc):
    tf.keras.backend.clear_session(); gc.collect()
    units      = trial.suggest_categorical("units",      [256, 384, 512, 768])
    ...
    try:
        ...
        if np.isfinite(val):
            ...
            print(
                ...
                f"PF={bt['pf']:.2f} | score={score:.4f} {mdd_flag}", flush=True,
            )
...
def print_summary_table():
    ...
    for tn, r in sorted(TRIAL_RESULTS.items()):
        killed = "🔴" if r["mdd"] < MAX_MDD else ("⚠️" if r["mdd"] < MDD_SOFT else "✅")
    ...
    print("\n" + "="*95)
    print("TRIAL COMPARISON SUMMARY  v11  (🔴=MDD killed | ⚠️=soft limit | ✅=target met)")
    print("="*95)
    print(df.to_string())
    print("="*95)
    if best_tn:
        ...
        print(f"★ BEST: T{best_tn:02d} | Score={b['score']:.4f} | "
              ...
              f"Sharpe={b['sharpe']:.2f} | MDD={b['mdd']:.2%} | PF={b['pf']:.2f}")
    print("="*95 + "\n", flush=True)
    joblib.dump(df, os.path.join(OUT_DIR, "trial_summary_v11.save"))
...
def print_stability_report(wfv_metrics):
    ...
    joblib.dump(
        ...
        os.path.join(OUT_DIR, "stability_report_v11.save"),
    )
...
# ══════════════════════════════════════════════════════════════════════════════
def train():
    print(f"[GRU-4H-v11] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | "
          ...
          f"LR=[{LR_LOW:.0e},{LR_HIGH:.0e}] | D=[{D_LOW},{D_HIGH}]", flush=True)
    ...
    cbs = [
        ...
        ModelCheckpoint(os.path.join(OUT_DIR, "model_v11_best_final.keras"),
    ...
    m.save(os.path.join(OUT_DIR, "model_v11_final.keras"))
    ...
    joblib.dump(stats,         os.path.join(OUT_DIR, "stats_v11.save"))
    joblib.dump(wfv_metrics,   os.path.join(OUT_DIR, "wfv_metrics_v11.save"))
    joblib.dump(bp,            os.path.join(OUT_DIR, "best_params_v11.save"))
    joblib.dump(TRIAL_RESULTS, os.path.join(OUT_DIR, "trial_results_v11.save"))
