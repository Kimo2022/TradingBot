...
print(f"[config] 4H-v5 | SEQ={SEQ_LEN} | NF={NF} (price={N_PRICE}/time={len(TIME_FEATS)}) | TRIALS={N_TRIALS}", flush=True)


# ── Custom Layers: Lambda-free, fully serializable via get_config ─────────────

@tf.keras.utils.register_keras_serializable(package="tft_v5")
class GluV(Layer):
...
class PosEncoding(Layer):
    ...
    def call(self, x):
        return x + tf.cast(tf.constant(self._pe), x.dtype)
    def get_config(self):
...
# ── Backtest: auto-trim for broadcast safety ──────────────────────────────────
...
def find_best_trial():
    if not TRIAL_RESULTS:
        print("[tracker] No trials.", flush=True); return None
    ...
    print(f"  loss={best['val_loss']:.6f} | Sharpe={best['sharpe']:.2f} | PF={best['pf']:.2f} | MDD={best['mdd']:.2%}", flush=True)
    print("=" * 72, flush=True)
    return best["model"]
...
def _try_save_best(model, score, sc, label="trial"):
    global BEST_SCORE
    if score > BEST_SCORE:
        BEST_SCORE = score
        ...
        print(f"[auto-save] ★ [{label}] New best score={score:.4f} → best_model_4H.keras", flush=True)
...
def train():
    print(f"[TFT-4H-v5] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | TRIALS={N_TRIALS}", flush=True)
    ...
    print(
        f"[seqs] train={len(Xtr):,} | val={len(Xv):,} | test={len(Xte):,} | sl={sl} | nf={nf_}",
        flush=True,
    )
