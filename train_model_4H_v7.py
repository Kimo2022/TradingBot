...
TRAIN_FRAC          = 0.70
VAL_FRAC            = 0.10
NEUTRAL_W_FACTOR    = 0.45   # dampen Neutral class weight → push model toward Long/Short
LOG_RET_PERIODS     = [3, 6, 12, 24]   # cumulative log-return horizons (4H bars)
...
def mkfeat(btc_df, eth_df):
    ...
    # --- Log-Return Statistical Stability: cumulative log returns over N bars ---
    log_c = np.log(c + EPS)
    for p in LOG_RET_PERIODS:
        df[f"log_ret_{p}"] = (log_c - log_c.shift(p)).astype(np.float32)

    # --- Daily Trend Context (4 features) ---
    daily_open  = o.resample("1D").first().reindex(df.index, method="ffill").ffill()
    daily_atr   = atr_raw.resample("1D").mean().reindex(df.index, method="ffill").ffill()
    daily_ema20 = c.resample("1D").last().ewm(span=20, adjust=False).mean()\
                   .reindex(df.index, method="ffill").ffill()
    daily_ema20_prev = daily_ema20.shift(6)  # ~1 day back on 4H grid
    df["daily_ret"]            = np.log((c + EPS) / (daily_open + EPS)).astype(np.float32)
    df["daily_trend_strength"] = ((c - daily_open) / (daily_atr + EPS)).clip(-5, 5).astype(np.float32)
    df["daily_ema_slope"]      = ((daily_ema20 - daily_ema20_prev) / (daily_ema20_prev + EPS)).astype(np.float32)
    df["price_vs_daily_open"]  = ((c - daily_open) / (c + EPS)).astype(np.float32)
    del daily_open, daily_atr, daily_ema20, daily_ema20_prev, log_c
...
def train():
    print(f"[GRU-4H-v7.1] GPUs={NGPU} | SEQ={SEQ_LEN} | NF={NF} | TRIALS={N_TRIALS}", flush=True)
    ...
    total = dist.sum()
    raw_w = total / (N_CLASSES * dist.astype(np.float64) + EPS)
    raw_w[1] *= NEUTRAL_W_FACTOR   # dampen Neutral: push model toward actionable signals
    # re-normalize so mean weight = 1.0
    raw_w = raw_w / raw_w.mean()
    cw = {0: float(raw_w[0]), 1: float(raw_w[1]), 2: float(raw_w[2])}
